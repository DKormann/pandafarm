...
export class PersonTableHandle {
  ...
  /**
   * Access to the `name` unique index on the table `person`,
   * which allows point queries on the field of the same name
   * via the [`PersonNameUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.person.name().find(...)`.
   *
   * Get a handle on the `name` unique index on the table `person`.
   */
  name = {
    // Find the subscribed row whose `name` column value is equal to `col_val`,
    // if such a row is present in the client cache.
    find: (col_val: string): Person | undefined => {
      for (let row of this.tableCache.iter()) {
